...
Here we provide a R Notebook template to generate a sequences variants table from FASTQ files generated by Paired-End Illumina sequencing using the Qiagen UPX transcriptomics library preparation kit (). For maximal reproducibility, we also provide a R project file and its associated packrat packages management directory.
...
Place your FASTQ files and the `Samples_barcodes.csv` file in a directory, then place this directory within the directory named `run_data`.
